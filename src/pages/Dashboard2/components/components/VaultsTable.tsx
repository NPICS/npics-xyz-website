...
function VaultsTable(props: IProps) {
  ...
  const numberToString = (val: BigNumber) => {
    if (!val) return;
    let factor = +new BigNumber(val.toString())
      .div(10 ** 18)
      .dp(0)
      .toString();
  ...
  const getNftActivities = async () => {
    ...
    try {
      ...
      if (result.code === 200 && orgData.length) {
        ...
        for (let i = 0; i < len; i++) {
          promiseArray.push(
            lendPool.getNftDebtData(orgData[i].nftAddress, orgData[i].tokenId)
          );
          promiseArray2.push(
            lendPool.getNftLiquidatePrice(
              orgData[i].nftAddress,
              orgData[i].tokenId
            )
          );
        }
        ...
        for (let i = 0; i < len; i++) {
          ...
          values1[i].healthFactor = new BigNumber(
            values1[i].healthFactor.toString()
          );
          values1[i].totalCollateral = new BigNumber(
            values1[i].totalCollateral.toString()
          );
          values1[i].availableBorrows = new BigNumber(
            values1[i].availableBorrows.toString()
          );
          values2[i].liquidatePrice = new BigNumber(
            values2[i].liquidatePrice.toString()
          );
          values2[i].paybackAmount = new BigNumber(
            values2[i].paybackAmount.toString()
          );
          newArray.push({
            ...values1[i],
            ...values2[i],
        ...
        for (let i = 0; i < len; i++) {
          const healthFactor = listData[i].status
            ? new BigNumber(0)
            : listData[i].healthFactor;
          const liquidatePrice = listData[i].status
            ? new BigNumber(0)
            : listData[i].liquidatePrice;
          const totalDebt = listData[i].status
            ? new BigNumber(0)
            : listData[i].totalDebt;
          dataSource.push({
            key: `${i}`,
