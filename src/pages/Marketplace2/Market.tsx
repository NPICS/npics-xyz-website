    ...
    const inner = async () => {
      try {
        const resp: any = await http.myPost(
          `/npics-nft/app-api/v2/nft/getCollections2`
        );
        if (resp.code === 200 && resp.data.length) {
          setListData(
            deserializeArray(CollectionList, JSON.stringify(resp.data))
          );
        }
      } catch (error) {}
    };
    ...
    >
      ...
      <Box zIndex={1}>
        <Box marginTop={"2.1rem"}>
          <Typography color={"#fff"} fontSize={"0.3rem"} fontWeight={"800"}>
            Collections
          </Typography>
        </Box>
        <Flex
        ...
        <Box marginTop={"0.1rem"} minHeight={"50vh"} marginBottom={"0.6rem"}>
          <Outlet />
        </Box>
      </Box>
    </Flex>
