...
export function VaultAprPop(props: { rewardAPR: number, interestAPR: number }) {
  ...
  return <Grid width={'6rem'} gridGap="0.2rem" borderRadius="0.2rem">
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">Reward APR (real-time)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">{(props.rewardAPR * 100).toFixed(2)}%</Typography>
    </Flex>
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">Interest APR (real-time)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">{-(props.interestAPR * 100).toFixed(2)}%</Typography>
    </Flex>

    <BorderBottom/>

    <Typography fontSize="0.14rem" fontWeight="500" color="#000">Vault APR = Reward APR + Interest APR</Typography>

    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">Rates shown are estimates, and fluctuate based
      on many different factors
      including collection floor price, trading volume, pool liquidity, token price, etc.</Typography>
    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">Funding fees collected by NPics are
...
export function DownPaymentPop(props: { listedPrice?: BigNumber, loanAmount?: BigNumber }) {
  ...
  return <Grid width={'6rem'} gridGap="0.2rem" borderRadius="0.2rem">
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">Listed Price (real-time)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500"
                  color="#000">{`${props.listedPrice?.div(10 ** globalConstant.bit).toFixed(2, 1)}ETH ` ?? '---'}{` ($${listedPrice})`}</Typography>
    </Flex>
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">Loan Amount (real-time)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500"
    ...
    <Typography fontSize="0.14rem" fontWeight="500" color="#000">Down Payment = Listed Price - Loan Amount</Typography>

    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">The system obtains the prime price based on
      the market listing price in real- time, And the amount paid will be used as down payment for your position</Typography>

  </Grid>
}

export const MintedNFTPop = <Grid gridGap="0.2rem" width="4.5rem">
  <Typography fontSize="0.14rem" fontWeight="500" color="#000">NEO NFT NPics Everlasting Option NFT</Typography>
  <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">NEO is NBPs voucher issued by the NPics on
...
export function DebtPop(props: { Principal?: BigNumber, Interest?: BigNumber, noInterest: string }) {
  ...
  return <Grid width={'6rem'} gridGap="0.2rem" borderRadius="0.2rem">
    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">Debt is the total of the principal and
    ...
    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">Please note that the interest will be settled
...
export function EstimatProfitPop(props: { purchaseFloorPrice?: BigNumber, ltv?: BigNumber, floorPrice?: BigNumber }) {
  ...
  return <Grid width={'6rem'} gridGap="0.2rem" borderRadius="0.2rem">
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">PFP (Purchase floor price history)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500"
                  color="#000">{`${purchaseFloorPrice?.div(10 ** globalConstant.bit).toFixed(2, 1)}ETH ` ?? '---'}{` ($${purchaseFloorPriceDollar})`}</Typography>
    </Flex>
    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">FP(real-time floor price)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500"
                  color="#000">{`${floorPrice?.div(10 ** globalConstant.bit).toFixed(2, 1)}ETH ` ?? '---'}{` ($${floorPriceDollar})`}</Typography>
    </Flex>

    <Flex alignItems='center' justifyContent="space-between">
      <Typography fontSize="0.14rem" fontWeight="500" color="#000">DP(Down Payment history)</Typography>
      <Typography fontSize="0.14rem" fontWeight="500"
    ...
    <Typography fontSize="0.14rem" fontWeight="500" color="#000">Profit Amount =(FP- PFP) * 1</Typography>
    <Typography fontSize="0.14rem" fontWeight="500" color="#000">Profit Rate = Profit Amount / DP * 100%</Typography>

    <Typography fontSize="0.14rem" fontWeight="500" color="rgba(0,0,0,.5)">The estimated position profit are calculated
