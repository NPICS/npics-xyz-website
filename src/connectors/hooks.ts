...
// Coinbase
export const [coinbase, coinbaseHooks] = initializeConnector<CoinbaseWallet>((actions) => {
...
export const [network, networkHooks] = initializeConnector<Network>((actions) => {
  return new Network({actions, urlMap: {1: nodeUrl()}, defaultChainId: 1})
})
...
export function useConnectors(selectedWallet: Wallet | undefined) {
  return useMemo(() => {
    ...
    connectors.push(
      ...WALLETS.filter((wallet) => wallet !== selectedWallet).map(getConnectorListItemForWallet)
    )
